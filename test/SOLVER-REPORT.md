...
更新日期：2026-07-27（第十六版：DFS 热路径整型索引镜像）
...
第十六版清除该桶（DFS 热路径整型索引镜像，语义对象保持权威、变更
位点双写，13/13 与重构前基线逐位一致）：7×7-8-7A 墙钟 19 档
26.0s→12.6s（−51.6%）、20 档 52.1s→25.3s（−51.4%），节点数不变；
自 P7 前基线累计 19 档 −63%。
...
## 第十六版：DFS 热路径整型索引镜像（本轮唯一性能优化）

### 实现（无运行时开关；回退单位是本 commit）

第十四版 profile 的最大剩余桶（字符串键动态查找 IC，≈33%）本轮清除：
语义对象保持权威不变——`placed`/`portBans`/各静态图照旧被
rememberSolution、deepest、zeroSafetyLookahead、legacy sk、
occupiedBarrierColors 消费——DFS 热路径全部改读以 `cellIdx = y*W+x`
为下标的数组镜像：

- 静态镜像：fixed / tunnel(含 pairIdx) / T-switch / autoswitch /
  Barrier / 触发器 / tsw 触发器 / meta / blank 标志 / 站台目标；
- 动态镜像（在既有变更位点双写）：`_placedArr` + `_placedCount` 计数
  （铺/回退/升级 5 个位点），`_portBanMask` 位掩码（addPortBan 及其
  undo），usage 表 `_useArr` 整体数组化；
- `Object.keys(placed).length` 的每节点/每候选调用全部换成计数器；
  occ 占格集合改整型；`pk()` 字符串仅在铺轨、autoswitch 记录、锁创建、
  站台拾取（有站台目标的格子才调用，语义门与
  `platformPickupForCar` 的空目标短路精确等价）等冷分支经
  `_cellKey[idx]` 查表取用；P7 权重读 `_placedArr`；P10 尾段读掩码
  （位映射与旧 Set 编码逐位相同）。

无 A/B 开关的理由：表示层替换会把每个热函数翻倍；回退单位是 commit。
正确性由更强的验收替代（下条）。

### 硬验收：13/13 与重构前基线逐位一致

对第十五版记录的 13 例基线（节点数/成本/结论）逐例复现：2,579 / 1,402 /
13,141 / 7,540 / 708 / 375 / 87 / 52 / 1 / 2 / 160,765 / 1,071 /
CSP-15 全部逐位相同；且 P10 off/on 双键格式仍互等——portBans 对象与
掩码的双写正确性被 legacy-sk 与热路径两条读路交叉验证。

### 7×7-8-7A 基准（P7 开、P10 开、seed 0、单 Worker）

| budget | 第十五版 | 本版 | 墙钟变化 | 节点 |
|---:|---|---|---:|---|
| 19 | 26,022ms | **12,603ms** | **−51.6%** | 7,258,797（不变） |
| 20 | 52,147ms | **25,342ms** | **−51.4%** | 14,226,560（不变） |

首候选 652ms → 363ms。每节点成本 ≈3.6µs → ≈1.74µs。legacy 键侧
（P10 off）也从 27.2s → 14.0s，说明收益来自索引化本身而非键格式交互。
自第十轮 P7 前基线累计：budget 19 穷尽 34.2s → 12.6s（−63%），
budget 20 从 15M 迭代内无法完成证明到 25.3s 单跑 `optimal-proven`。
金丝雀 10、协议 2+5、集成 4、全门禁绿。

### 路线

profile 的两大桶（字符串状态键、字符串键 IC）已依次清除。下一步重新
profile 决定是否还有值得做的单变量（p7Field/dfs 自身、分配），或转向
P12 一般化（环族参数化）的设计轮。

...
外推到 Barrier。P5②（连通性冻结形式）已于第十一版做适用性探针：把全部
Barrier 格视为墙的最坏图下，7×7-8-7A / 7×8-8-7 / 8×9-6-9D 的全部触发器
仍对全部车连通，冻结前提可证永不成立，故未实现（见 SOLVER-REPORT
第十一版）。两题终点入口邻格本身是 Barrier，难度来自开关时序而非连通性；
若新语料出现“墙能切断触发器”的拓扑应重新评估。不要把这两个结论合并写成
“P5 对 Barrier 无效”。
